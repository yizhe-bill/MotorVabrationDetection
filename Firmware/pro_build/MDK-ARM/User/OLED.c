...
// {
//     uint8_t data[2] = {0x00, Command};  // 控制字节为 0x00，表示写命令
...
/********************************************************************
   * @brief  : OLED写数据
   * @param  :Data 要写入数据的起始地址；Count 要写入数据的数量
   * @retval : 避免使用MALLOC函数，以免内存碎片化，当然这个也能用
*********************************************************************/
...
//     if (buffer == NULL)
//         return;
//     buffer[0] = 0x40;  // 控制字节为 0x40，表示写数据
